...
I entered the TestSprite Season 2 hackathon with a straightforward goal: use their AI-powered testing agent to validate [homelab_mcp](https://github.com/washyu/homelab_mcp), my open-source MCP server for homelab infrastructure management. What I didn't anticipate was that the most interesting engineering challenge wouldn't come from the server itself — it would come from the protocol mismatch between where TestSprite lives and where MCP servers live.
